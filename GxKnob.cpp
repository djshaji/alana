...
void _gx_knob_draw_shtuff(GtkWidget *widget, cairo_t *cr, GdkRectangle *image_rect, gdouble knobstate)
{
    ...
    // style definitions
    gint ind_radius = 250, ind_width = 250, ind_length = 250, ring_radius = 250, ring_width = 250;
    ...
    GdkRGBA color_;
    gtk_style_context_get_color(style, &color_);
...
static GdkPixbuf * load_icon (const char * icon, int size) {
    GtkIconTheme * theme = gtk_icon_theme_get_for_display (gdk_display_get_default ());
    GtkIconPaintable * p = gtk_icon_theme_lookup_icon (theme, icon, NULL, size, 1, GTK_TEXT_DIR_NONE, GTK_ICON_LOOKUP_FORCE_REGULAR);
    GFile * f = gtk_icon_paintable_get_file (p);
    char * c = g_file_get_path (f);
    GdkPixbuf * pixbuf = gdk_pixbuf_new_from_file (c, NULL);
    g_free (c);
    g_free (f);
    return pixbuf ;
...
static gboolean gx_knob_draw(GtkWidget *widget, cairo_t *cr)
{
    g_assert(GX_IS_KNOB(widget));
    GdkRectangle image_rect, value_rect;
    GdkPixbuf *pb = load_icon(get_stock_id(widget), -1);
    if (GDK_IS_PIXBUF (pb)) {
        gint fcount;
        get_image_dimensions (widget, pb, &image_rect, &fcount);
