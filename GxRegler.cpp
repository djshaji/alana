...
static gboolean gx_regler_spinner_output(GtkSpinButton *spinner, GxRegler *regler) {
	...
	//~ gtk_entry_buffer_set_text (gtk_entry_get_buffer (spinner), fmt);
	g_free(fmt);
    return TRUE;
}
...
static gboolean gx_regler_value_entry(GxRegler *regler, GdkRectangle *rect, GdkEventButton *event)
{
	...
	gtk_popover_set_child ((GtkPopover *)popover, spinner);

	g_signal_connect_object(spinner, "activate", G_CALLBACK(gtk_widget_hide),
				popover, (GConnectFlags)(G_CONNECT_AFTER|G_CONNECT_SWAPPED));
    g_signal_connect((GtkWidget *)popover, "hide", G_CALLBACK(gtk_widget_hide), popover);
