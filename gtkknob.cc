...
static void gtk_knob_destroy(GObject *object_)
{
    GtkefnameKnob *knob = GTK_EFNAME_KNOB(object_);
    ...
    //~ G_OBJECT_CLASS(gtk_knob_parent_class)->destroy(object_);
}
...
static void gtk_knob_class_init (GtkefnameKnobClass *klass)
{
    ...
    GObjectClass *widget_class = G_OBJECT_CLASS(klass);
...
static void gtk_knob_init (GtkefnameKnob *knob)
{
    ...
    gtk_widget_set_size_request(GTK_WIDGET(widget), priv -> knob_x, priv->knob_y); 
    //~ requisition.width = priv->knob_x;
    //~ requisition.height = priv->knob_y;
}
